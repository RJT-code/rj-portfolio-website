  ...
  return (
    <footer className="border-t border-border bg-midnight-navy py-8 text-white">
      <Container className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
        <a className="inline-flex min-h-11 items-center gap-3 rounded-lg" href="#home" aria-label="RJ, back to home">
        ...
        <nav className="flex flex-wrap gap-x-5 gap-y-2" aria-label="Footer navigation">
          {footerNavigation.map(([label, href]) => (
            <a key={label} className="inline-flex min-h-11 items-center text-sm text-white/70 transition-colors hover:text-white" href={href}>{label}</a>
          ))}
        </nav>
