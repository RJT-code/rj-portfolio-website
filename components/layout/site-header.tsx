...
function Brand() {
  return (
    <a className="group inline-flex min-h-11 items-center gap-3 rounded-lg" href="#home" aria-label="RJ, home">
    ...
    <header className="sticky top-0 z-50 border-b border-border/80 bg-background/90 backdrop-blur-xl">
      <Container className="flex min-h-18 items-center justify-between gap-5">
        ...
        <nav className="hidden items-center gap-1 xl:flex" aria-label="Primary navigation">
          {navigation.map(([label, href]) => <a key={label} className="nav-link" href={href}>{label}</a>)}
        </nav>
        <a className="button-primary hidden xl:inline-flex" href="#contact">Let&apos;s talk</a>
        <details className="mobile-menu relative xl:hidden">
          ...
          <div className="absolute right-0 top-[calc(100%+0.75rem)] w-[min(20rem,calc(100vw-2.5rem))] rounded-xl border border-border bg-surface-elevated p-3 shadow-soft">
            <nav className="grid" aria-label="Mobile navigation">
              {navigation.map(([label, href]) => <a key={label} className="flex min-h-11 items-center rounded-lg px-3 text-sm font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground" href={href}>{label}</a>)}
              <a className="button-primary mt-2" href="#contact">Let&apos;s talk</a>
            </nav>
          </div>
        </details>
      </Container>
